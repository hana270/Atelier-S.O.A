...
export class UpdateIngredientComponent implements OnInit {
  currentIngredient = new Ingredient();  // L'ingrédient actuel
  recettes: Recette[] = [];  // Liste des recettes
  updatedRecId?: number;  // ID de la recette à associer
  myImage!: string;  // Image de prévisualisation
  uploadedImage!: File;  // Image téléchargée
  isImageUpdated: Boolean = false;  // État de mise à jour de l'image
  ...
  ngOnInit(): void {
    this.loadRecettes();  // Charger les recettes
    this.loadIngredient();  // Charger l'ingrédient à modifier
  }
